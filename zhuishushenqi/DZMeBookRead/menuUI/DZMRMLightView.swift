...
class DZMRMLightView: DZMRMBaseView {
    ...
    override func layoutSubviews() {
        ...
        let height1 = height - 34
        // 标题
        titleLabel.frame = CGRect(x: 0, y: 0, width: 45, height: height-34)
        
        // 类型
        let typeLabelW:CGFloat = 32
        let typeLabelH:CGFloat = 16
        typeLabel.frame = CGRect(x: width - typeLabelW - DZMSpace_1, y: (height1 - typeLabelH) / 2, width: typeLabelW, height: typeLabelH)
        
        // 进度条
        let sliderX = titleLabel.frame.maxX + DZMSpace_1
        let sliderW = typeLabel.frame.minX - DZMSpace_1 - sliderX
        slider.frame = CGRect(x: sliderX, y: 0, width: sliderW, height: height-34)
    }
