...
class DZMRMTopView: DZMRMBaseView {
    ...
    override func layoutSubviews() {
        ...
        // 返回按钮
        back.frame = CGRect(x: 0, y: getStatusBarHeight(), width: buttonW, height: height - getStatusBarHeight())
        
        // 书签按钮
        mark.frame = CGRect(x: width - buttonW, y: getStatusBarHeight(), width: buttonW, height: height - getStatusBarHeight())
    }
}
